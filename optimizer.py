...
def build_capex_breakdown(sys_type, scope, base_tr, tes_trh, brine_tr, rates, is_air_cooled=False, tank_shape="Cylindrical Tank"):
    ...
    if sys_type == "Conventional":
        ...
        b["Water Infra"] = 0.0 if is_air_cooled else base_tr * rates.get("water_infra_rate", 1200.0)
        b["Transformer"] = base_tr * 0.8 * rates.get("transformer_rate", 3500.0)
        b["DG Set"] = base_tr * 0.8 * rates.get("dg_set_rate", 12500.0)
        
    elif sys_type == "PCM":
        b["Chiller Equip."] = brine_tr * rates.get("brine_chiller_rate", 25000.0)
        if is_greenfield: b["Chiller Equip."] += base_tr * (rates.get("ac_chiller_rate", 24000.0) if is_air_cooled else rates.get("base_chiller_rate", 22000.0))
        ...
        if is_greenfield: b["Chiller Equip."] = base_tr * (rates.get("ac_chiller_rate", 24000.0) if is_air_cooled else rates.get("base_chiller_rate", 22000.0))
        b["TES System"] = tes_trh * rates.get("stratified_tes_rate", 18000.0)
    ...
    b["Indirects / AMC"] = subtotal * rates.get("indirects_pct", 0.30)
