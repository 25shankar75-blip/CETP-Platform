"""
CETP Digital Twin - Iterative OPEX Maximizer
File: optimizer.py
"""
import numpy as np
import pandas as pd
from physics_engine import simulate_24h_plant, simulate_pcm_tes_24h, simulate_stratified_tes_24h, fetch_live_weather_wbt
from financial_engine import calc_capex_breakup, calc_payback_and_roi

def optimize_tes_plant(df_24h: pd.DataFrame, scope: str, peak_tr: float, audit_config: dict, rates: dict, location: str, fleet_df: pd.DataFrame):
    
    weather = fetch_live_weather_wbt(location)
    is_any_ac = any("Air-Cooled" in str(t) for t in fleet_df["Chiller Type"]) if not fleet_df.empty else False

    base_sim = simulate_24h_plant(df_24h, scope, audit_config, fleet_df, weather)
    base_capex = calc_capex_breakup(scope, "Conventional", peak_tr, 0, 0, rates)["total_capex"]
    base_opex = base_sim["annual_opex"]
    
    # Scale up to immense tank sizes (25,000 TRh) to handle Mondelez benchmark sizes
    max_search_trh = min(25000.0, peak_tr * 12.0)
    test_capacities = np.linspace(500.0, max_search_trh, 30)
    ...
    for tes_trh in test_capacities:
        ...
        sim_pcm = simulate_pcm_tes_24h(df_24h, tes_trh, charge_chiller_tr, fleet_installed_tr=peak_tr, weather_data=weather, is_any_ac=is_any_ac)
        ...
        capex_delta = pcm_capex - base_capex if scope != "Brownfield (Retrofit)" else pcm_capex
    ...
    if best_pcm is None:
        # Fallback to Mondelez Anchor Baseline
        tes_trh = 3017.0
    ...
    for tes_trh in test_capacities:
        sim_strat = simulate_stratified_tes_24h(df_24h, tes_trh, fleet_installed_tr=peak_tr, weather_data=weather, is_any_ac=is_any_ac)
        ...
        capex_delta = strat_capex - base_capex if scope != "Brownfield (Retrofit)" else strat_capex
        payback, roi = calc_payback_and_roi(capex_delta, opex_savings)
