...
def build_capex_breakdown(sys_type, scope, fleet_tr, tes_trh, charge_chiller_tr, rates, is_air_cooled=False):
    b = {
        "Chiller Equip.": 0, "TES Tank": 0, "PCM Media": 0, "Pumps & PHE": 0, 
        "Electrical": 0, "Water Infra": 0, "Transformer": 0, "DG Set": 0
    }
    
    if sys_type == "Conventional":
        if scope == "Brownfield (Retrofit)": 
            return {"Total CAPEX": 0.0, "Breakdown": b}  # SUNK COST BASELINE
        c_rate = rates["ac_chiller_rate"] if is_air_cooled else rates["base_chiller_rate"]
        b["Chiller Equip."] = fleet_tr * c_rate
        b["Pumps & PHE"] = 0.0 if is_air_cooled else fleet_tr * 2500
        ...
        b["Pumps & PHE"] = (tes_trh / 8.0) * 1500
        b["Electrical"] = (tes_trh / 8.0) * 1000
        b["Water Infra"] = (tes_trh / 8.0) * 800.0

    subtotal = sum(b.values())
    indirects = subtotal * rates["indirects_pct"]
...
def optimize_plant(df_comp, load_arr, tar_arr, wbt_arr, proj_scope, audit_cfg, rates, running_days):
    ...
    for trh in search_space:
        ...
        if pb <= 4.0 and sav > best_pcm["opex_savings"]:
            best_pcm = {"tes_trh": trh, "chiller_tr": c_tr, "sim": sim, "cap": cap, "opex_savings": sav, "payback": pb, "num_tanks": int(np.ceil(trh / 25000.0)), "water_cost": w_cost}
    ...
    # 3. Stratified Optimization Loop
    ...
    # 4. Apply DG Outage Fuel & Grid Offset
    dg_cost_baseline = rates["dg_diesel_cost_kwh"] * (np.mean(sim_conv["comp_kw"]) + np.mean(sim_conv["chw_pri_kw"]) + np.mean(sim_conv["chw_sec_kw"])) * rates["daily_outage_hrs"] * running_days
    dg_cost_tes_p = rates["dg_diesel_cost_kwh"] * (np.mean(best_pcm["sim"]["chw_sec_kw"])) * rates["daily_outage_hrs"] * running_days
    dg_cost_tes_s = rates["dg_diesel_cost_kwh"] * (np.mean(best_strat["sim"]["chw_sec_kw"])) * rates["daily_outage_hrs"] * running_days
    
    avg_tariff = np.mean(tar_arr)
    grid_offset_p = (dg_cost_baseline - dg_cost_tes_p) * (avg_tariff / rates["dg_diesel_cost_kwh"]) if proj_scope == "Brownfield (Retrofit)" else 0.0
    grid_offset_s = (dg_cost_baseline - dg_cost_tes_s) * (avg_tariff / rates["dg_diesel_cost_kwh"]) if proj_scope == "Brownfield (Retrofit)" else 0.0
